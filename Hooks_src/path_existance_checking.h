...
namespace pathexsitance{
    ...
    public:
        ...
        Path_checker(std::vector<std::vector<int>> &map, std::vector<std::pair <int,std::vector<float>>> *restrictz,
                         double imax_x, double imax_y, double resol):m_map(map), max_x(imax_x),max_y(imax_y), 
                         resolution(resol){
                m_restrictz = restrictz;
            };
        ~Path_checker(){
